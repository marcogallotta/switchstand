...
from switchstand.test_grant import parser, test_database_url
...
def test_database_target_cannot_be_overridden(url):
    with pytest.raises(ValueError, match="switchstand_test"):
        test_database_url({"TEST_DATABASE_URL": url})


def test_database_target_reaches_driver():
    url = test_database_url({"TEST_DATABASE_URL": "postgresql+psycopg:///switchstand_test"})
