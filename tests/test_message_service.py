...
async def service(
    ...
    return MessageService(resolve, state, grants, messages, generation)
...
async def test_replacement_recovers_same_received_delivery_identity(engine):
    ...
    sender_service = await service(sender, state, grants, messages, "chatgpt-1")
    old_recipient = await service(recipient, state, grants, messages, "run-1")
    ...
    replacement = await service(recipient, state, grants, messages, "run-2")
    ...
    stale_old = await old_recipient.receive(delivery)
