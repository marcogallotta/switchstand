...
def test_context_provisions_before_codex_without_provider_token(monkeypatch, tmp_path):
    ...
    def fake_preflight(repo, private_writer, env):
        assert private_writer == writer
        events.append(("preflight", repo, private_writer))
        return str(private_writer / ".switchstand-check" / ("a" * 64)), "a" * 64
    ...
    assert [event[0] for event in events] == ["writer", "preflight", "provision", "codex"]
    assert events[1] == ("preflight", tmp_path, writer)
...
def test_failed_preflight_stops_before_provision_or_codex(monkeypatch, tmp_path):
    ...
    def fail(control, private_writer, env):
        assert private_writer == writer and writer.is_dir()
        raise ValueError("private environment unavailable")
    ...
    monkeypatch.setattr(context, "provision", forbidden)
    monkeypatch.setattr(context, "create_writer", create_writer)
    monkeypatch.setattr(context, "managed_codex_home", forbidden)
    monkeypatch.setattr(context, "supervise", forbidden)
    monkeypatch.setattr(context.os, "execv", forbidden)
    with pytest.raises(ValueError, match="private environment unavailable"):
        context.run("1218483858041754")
    assert list(tmp_path.iterdir()) == [writer]
