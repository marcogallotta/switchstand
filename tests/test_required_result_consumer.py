...
from switchstand.grants import PrincipalContext, ProtectedAppend, WorkGrant
...
async def test_required_result_real_write_readback_closes_and_replays_without_duplicate(
    ...
    first = await service.required_result_save(obligation_id, action)
    ...
    replay = await service.required_result_save(obligation_id, action)
...
async def test_crash_after_verified_write_replays_effect_and_finishes_without_duplicate(
    ...
    interrupted = await service.required_result_save(obligation_id, action)
...
async def test_same_obligation_rejects_changed_result_and_changed_grant_currentness(
    result_engine: AsyncEngine,
) -> None:
    ...
    stale_source = await service.required_result_save(obligation_id, first)
    assert stale_source.status == "stale" and provider.sends == 0
    stored = await lifecycle.repository.get(obligation_id)
    assert stored is not None and stored.state is ProfileState.PERSIST_REQUIRED
    ...
    conflict = await service.required_result_save(obligation_id, changed)
