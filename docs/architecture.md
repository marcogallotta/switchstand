...
PostgreSQL contains one table: `work_handles`. Local configuration lives in
`~/.config/switchstand/.env`; Compose passes it to the controller. The MCP surface exposes no raw provider
operations.
