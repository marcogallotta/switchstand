...
async function stopAgent(agent) {
  try {
    const prepared = await request("/api/native-stop/prepare", { agentRef: agent.agentRef }, true); if (prepared.code !== "prepared") {
      ...
      stopState.set(agent.agentRef, await request("/api/native-stop/commit", { confirmationRef: prepared.confirmationRef }, true));
    }
...
async function checkStop(agentRef) {
  const current = stopState.get(agentRef); try {
    stopState.set(agentRef, await request("/api/native-stop/status", { operationRef: current.operationRef }, true));
...
function renderNative(model) {
  ...
  descriptionHost.textContent = "Observed evidence with exact-turn emergency stop."; headlineFactsHost.textContent = "Observed state only · exact cancellation requests";
  ...
  const agents = model.agents.map((agent) => {
    ...
    const stop = stopState.get(agent.agentRef); if (stop) row.append(el("p", "muted", `Stop outcome: ${stop.outcome}`));
    if (agent.status === "active" && !["requested", "confirmed"].includes(stop?.outcome)) {
      const button = el("button", "button button--danger", "Stop current turn"); button.type = "button"; button.addEventListener("click", () => stopAgent(agent));
      row.append(button);
    }
    if (["requested", "unknown"].includes(stop?.outcome) && stop.operationRef) {
      const check = el("button", "button", "Check stop outcome"); check.type = "button"; check.addEventListener("click", () => checkStop(agent.agentRef));
      row.append(check);
    }
    return row;
  });
