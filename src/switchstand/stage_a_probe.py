...
def collect_evidence(
    ...
    *,
    ...
    """Collect bounded snapshots and notifications without loading or mutating threads."""
    ...
    return {
        ...
        "notificationEvidence": {
            "delivery": "initializedAppServerConnection",
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        ...
        description=(
            ...
            "The probe never resumes, loads, starts, steers, interrupts, or otherwise mutates a thread."
        ),
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        result = _failure("transport_failure", "App Server transport failed")
        print(json.dumps(result, sort_keys=True))
        return 3
    except ProbeEvidenceError as exc:
        result = _failure(exc.code, str(exc))
        print(json.dumps(result, sort_keys=True))
        return 4
        ...
        if "WebSocket" in str(exc) or "app-server closed" in str(exc):
            result = _failure(
                "transport_failure", "App Server connection failed or closed (RuntimeError)"
            )
            exit_code = 3
        else:
            result = _failure(
        ...
        result = _failure("evidence_unavailable", "protocol evidence was not valid")
        print(json.dumps(result, sort_keys=True))
        return 4
    finally:
