...
- Managed runs use their bound provider-neutral Switchstand work/history/event capability (`work_get`, `work_history`, `work_event`, `work_append`) for routine work/source access. A temporary raw Asana bridge needs an exact grant and ends once the work is Switchstand-bound.
...
- Load the active inbox with `work_get`, then page `work_history` at the returned revision until `next_cursor=null`. The first page is not necessarily newest. Use `work_event` when an exact event must be reread. On `stale`, call `work_get` again and restart the affected history read.
